...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos) {
    ...
    const float startsat = 1.0f / (2 * SAMPLE_SIZE);

    for (int x = std::max(0, int(std::floor(l))); x <= rr; ++x) {
        for (int y = std::max(0, int(std::floor(d))); y <= uu; ++y) {

            const int depth_index = (x + width * (height - y - 1));

            if (insideTriangle(x + startsat, y + startsat, t.v)) {
                auto [alpha, beta, gamma] = computeBarycentric2D(x + startsat, y + startsat, t.v);
                ...
                if (depth_buf[depth_index] > -z_interpolated) {
                    ...
                    frame_buf[depth_index] = pixel_color;
                    set_pixel({x, y}, frame_buf[depth_index]);
                }
            }
        }
    }
